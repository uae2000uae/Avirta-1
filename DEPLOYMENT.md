...
This section provides instructions for deploying the Avirta web application using Docker.
...
```bash
docker run -d -p 8080:8080 --name avirta-app avirta:latest
```

This command:
- Runs the container in detached mode (`-d`)
- Maps port 8080 on the host to port 8080 in the container (`-p 8080:8080`)
- Names the container "avirta-app" (`--name avirta-app`)
- Uses the "avirta:latest" image
...
```bash
docker stop avirta-app
```

2. Remove the container:

```bash
docker rm avirta-app
```
...
For a more manageable deployment, you can use Docker Compose. Create a `docker-compose.yml` file in the project root:
